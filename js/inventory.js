document.addEventListener('DOMContentLoaded', () => {
    ...
    const cargarInventario = async () => {
        try {
            tablaPrincipal.innerHTML = '<tr><td colspan="7" style="text-align:center;">Cargando inventario...</td></tr>';
            ...
            tablaPrincipal.innerHTML = '<tr><td colspan="7" style="text-align:center; color:#dc3545;">Error al cargar los datos. Verifica la conexión a la API.</td></tr>';
        }
    };
    ...
    const renderizarTablaPrincipal = (items) => {
        ...
        if (!items || items.length === 0) {
            tablaPrincipal.innerHTML = '<tr><td colspan="7" style="text-align:center;">No se encontraron productos.</td></tr>';
            return;
        }
    ...
    btnFilter.addEventListener('click', () => {
        ...
        // Filtrar por Nombre
        if (searchTerm !== '') {
            filtrados = filtrados.filter(p => p.producto.toLowerCase().includes(searchTerm));
        }
    ...
    const abrirModalLotes = (id, nombre) => {
        ...
        const lotesDelProducto = lotesGlobales.filter(lote => lote.productId === id);
    ...
    const renderizarLotes = (listaLotes) => {
        ...
        if (!listaLotes || listaLotes.length === 0) {
            tbodyLotes.innerHTML = '<tr><td colspan="4" style="text-align:center;">No hay lotes disponibles para este producto.</td></tr>';
            return;
        }

        listaLotes.forEach(lote => {
            const fechaVencimiento = new Date(lote.fechaVencimiento).toLocaleDateString('es-NI');
